...
public class GiftRepeatItemView extends LinearLayout {
    ...
    public void showGift(GiftInfo giftInfo, String repeatId, UserProfile profile) {
        giftId = giftInfo.giftId;
        userId = profile.getAccount();
        this.repeatId = repeatId;

        if (getVisibility() == INVISIBLE) {
            totalNum = 1;
            //所有动画结束之后，
            String avatarUrl = profile.getHeader();
    ...
    public boolean isAvaliable(GiftInfo giftInfo, String repeatId, UserProfile profile) {
        boolean sameGift = giftId == giftInfo.giftId;
        boolean sameRepeat = this.repeatId.equals(repeatId);
        boolean sameUser = this.userId.equals(profile.getAccount());
